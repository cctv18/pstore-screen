...
{
	struct kmsg_dump_iter *iter = ctx;

	return READ_ONCE(iter->cur_seq);
}
				...
				u32 capacity, u32 *len)
{
	...
	if (syslog != 1U)
		return -EINVAL;
	got = kmsg_dump_get_line(iter, true, (char *)line,
					capacity, &line_len);
...
{
	enum kmsg_dump_reason reason = detail ? detail->reason : KMSG_DUMP_UNDEF;
#else
static void ps_log_kmsg_dump(struct kmsg_dumper *dumper,
			     enum kmsg_dump_reason reason)
{
	...
	pstore_screen_printk_snapshot_window(&iter);
	start_seq = READ_ONCE(iter.cur_seq);
	end_seq = READ_ONCE(iter.next_seq);
	ret = ps_log_capture_frozen_window(start_seq, end_seq, &ps_log_kmsg_ops,
					   &iter, ps_log_kmsg_scratch,
